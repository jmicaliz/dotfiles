...
Reorganize the `permissions` arrays in a Claude Code `settings.json`. Target file: the argument
passed to the command, or `./configs/.claude/settings.json` if none is given (or the settings.json
the user has open).

Follow this exact workflow:

1. Read the global settings file at `~/.claude/settings.json`. If it doesn't exist or has no
...
7. Separate visually distinct clusters with a single blank line (JSON permits whitespace between array
   ...
   > Note: blank lines are legal JSON but will be stripped by most auto-formatters (`jq`,
   > `prettier --parser json`, IDE save-on-format). If you re-run a formatter over this file,
   > re-run `/organize-permissions` afterwards.
